    ...
    <div>
      {optedIn ? (
        <div className="flex bg-white border border-[#BAC4D6] text-[#657795] font-normal justify-center text-sm w-full py-3 px-6 rounded-lg">
          <p className="pl-2">Opted-In</p>
        </div>
      ) : (
        <button
          className="flex gap-2 bg-[#E20880] hover:opacity-90 border text-[#fff] font-normal text-sm w-full py-3 px-6 rounded-lg"
          onClick={() => installSnap()}
        >
          <Image src={MetaMask} alt="MetaMask Logo" height="20" width="20" />
          <p className="">Opt-In With Snap</p>
        </button>
      )}
    </div>
  );
